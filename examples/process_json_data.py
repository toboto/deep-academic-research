...
from deepsearcher.tools.log import info, error, color_print, set_dev_mode, set_level
...
def process_json_data(input_file: str, output_file: str) -> None:
    ...
    for _, item in tqdm(enumerate(data), total=total_records, desc="Processing records"):
        ...
        if "authors" in item:
            ...
            item["corresponding_authors"] = list(set(corresponding_authors_list)) if corresponding_authors_list else []
            item["corresponding_author_ids"] = list(set(corresponding_author_ids)) if corresponding_author_ids else []
        else:
    ...
    # Prompt next steps
    color_print("\nNext steps:")
    color_print("1. You can use create_json_vector_db.py script to import the processed data into vector database")
    color_print(f"   Example: python examples/create_json_vector_db.py --file {output_file} --collection json_articles")
    color_print(f"   \t\t--collection-description 'Academic articles dataset imported from {os.path.basename(input_file)}'")
    color_print("2. Or run without parameters to use default settings: python examples/create_json_vector_db.py")
...
def main():
    ...
    parser.add_argument('input_file', nargs='?', help='JSON file path to process, must provide this argument or use --sample')
    parser.add_argument('-o', '--output', help='Output file path after processing, defaults to input filename with _processed.json')
    parser.add_argument('--sample', action='store_true', help='Process small sample file (articles_sample.json) instead of complete file')
    ...
    if args.input_file:
        ...
        default_output = os.path.join(os.path.dirname(input_file), f"{input_name}_processed{input_ext}")
        ...
        output_file = os.path.join(project_root, "examples", "data", "articles_sample_processed.json")
